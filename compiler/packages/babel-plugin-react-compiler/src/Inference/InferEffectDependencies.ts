...
export function inferEffectDependencies(
  ...
  /**
   * This pass runs before all pruning passes (such as PruneNonReactiveDeps)
   * which operate on reactive function form.
   * See the `infer-effect-deps/pruned-nonreactive-obj` fixture
   */
  ...
  for (const [, block] of fn.body.blocks) {
    ...
    let rewriteInstrs = new Map<InstructionId, Array<Instruction>>();
      ...
      if (value.kind === 'FunctionExpression') {
        ...
        if (fnExpr != null) {
          ...
          for (const dep of scopeInfo.deps) {
            if (reactiveIds.has(dep.identifier.id)) {
...
/**
 * This should only be called for actually reactive dependencies. See the
 * `infer-effect-deps/pruned-nonreactive-obj` fixture
 */
